...
import { Sub2APIKeySelector } from '@/components/settings/provider/Sub2APIKeySelector'
...
import { getSub2APIModels } from '@/packages/sub2api'
import { getSub2APIGroupIdFromProviderId, isSub2APIGroupProvider } from '@/packages/sub2api-provider'
...
function ProviderSettings({ providerId }: { providerId: string }) {
  ...
  const isSub2APIGroup = !!baseInfo && isSub2APIGroupProvider(baseInfo.id)
  const sub2apiGroupId = baseInfo ? getSub2APIGroupIdFromProviderId(baseInfo.id) : undefined
  const supportsSub2APIKeySelection =
    !!baseInfo &&
    !isOAuthOnlyProvider &&
    ![
  ...
  const handleFetchModels = async () => {
    ...
    try {
      ...
      const modelList = isSub2APIGroup
        ? await getSub2APIModels(providerSettings?.apiKey || '')
        : await getModelSettingUtil(baseInfo.id, baseInfo.isCustom ? baseInfo.type : undefined).getMergeOptionGroups({
      ...
      if (modelList.length) {
        if (isSub2APIGroup) {
          setProviderSettings({ models: modelList })
          addToast(`已同步 ${modelList.length} 个模型`)
        } else {
      ...
      addToast(error instanceof Error ? error.message : '模型获取失败')
      setFetchedModels(undefined)
      setFetchingModels(false)
    }
  }
  ...
  return (
    <Stack key={baseInfo.id} gap="xxl">
      <Flex gap="xs" align="center">
        ...
        {baseInfo.isCustom && !isSub2APIGroup && (
          <PopoverConfirm
      ...
      {baseInfo.isCustom && !isSub2APIGroup && language === 'zh-Hans' && (
        <Flex>
          <ScalableIcon icon={IconHelpCircle} />
          <Text span size="xs" c="chatbox-tertiary">
            <a href="https://usa0.top/help-center" target="_blank" rel="noopener">
              {t('Setup guide')}
            </a>
          </Text>
        </Flex>
      ...
      <Stack gap="xl">
        ...
        {supportsSub2APIKeySelection && (
          <Stack gap="xxs" style={isOAuthActive ? { opacity: 0.5 } : undefined}>
            ...
            <Flex gap="xs" align="flex-start">
              <Sub2APIKeySelector
                providerType={baseInfo.type}
                groupId={sub2apiGroupId}
        ...
        {/* API Host */}
        {!supportsSub2APIKeySelection && BUILTIN_API_HOST_PROVIDERS.has(baseInfo.id) && (
        ...
        {baseInfo.isCustom && !supportsSub2APIKeySelection && (
          <>
        ...
        <Stack gap="xxs">
          <Flex justify="space-between" align="center">
            ...
            <Flex gap="sm" align="center" justify="flex-end">
              ...
              >
                {isSub2APIGroup ? '重置' : t('Reset')}
              </Button>

              <Button
                loading={fetchingModels}
                disabled={supportsSub2APIKeySelection && !providerSettings?.sub2apiKeyId}
                variant="light"
                color="chatbox-gray"
                c="chatbox-secondary"
                size="compact-xs"
          ...
          <ModelList
            ...
            emptyText={isSub2APIGroup ? '暂无模型，请先获取模型。' : undefined}
            showActions={true}
            showSearch={false}
        ...
        <AdaptiveModal
          ...
          title={isSub2APIGroup ? '模型' : t('Models')}
          centered={true}
        >
          <ModelList
