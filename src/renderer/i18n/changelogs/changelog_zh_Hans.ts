...
4. 图片生成功能重构为独立工具页面
5. ZeroBox AI 设置从提供方移至顶级设置，更便于访问
6. 全新对话式新手引导
...
1. 修复 Latex 渲染问题
2. 修复左边栏顶部不能拖动的问题
3. 修复 ZeroBoxAI 错误信息展示
4. 增加初始化过程日志展示

### v1.12.0 - 2025.04.24

1. ZeroBox AI 支持 Gemini 多模态输出
...
1. 新增联网问答功能。现在 ZeroBox AI Models 和 Gemini 2.0 flash(API) 可以参考互联网的实时信息进行回答，并在消息中列出参考信息来源
2. 新增查看消息生成时的首字延迟时间（first-token latency），可在设置中开启
3. 新增发送图片的预处理能力。现在你可以发送更多格式的图片（例如 svg、gif），ZeroBox 会自动转化成模型可以接收的图片格式，并且自动调节图片大小以符合模型要求
4. 双击话题名称可以快速弹出话题列表
...
1. 现在你可以发送网页链接。ZeroBox 将自动获取网页内容并将其包含在聊天上下文中。适用于所有模型。
2. 现在你可以向任何模型发送文本文件。ZeroBox 将在本地解析文件内容并将其包含在聊天上下文中。
...
1. 现在 ZeroBox AI 服务的用户可以选择更多的模型
2. 新增阿拉伯语（العربية）
3. 优化了 latex 的样式
4. 修复了小概率情况下丢失数据的问题
5. 修复了其他一些小问题
...
- ZeroBox 网页版本现在也能访问本地 Ollama 服务。
...
3. 现在你从 ZeroBox 复制的文本，将会以纯文本的形式复制到剪贴板（不再包含背景颜色，这个长期存在的小bug终于修复了）
...
- 新增图片生成功能（Image Creator)，现在你可以在 ZeroBox 中生成图片了。由 Dall-E-3 模型提供支持。
- 优化一些交互问题
...
- 内置AI服务 “ZeroBox AI”，开箱即用，直接高速访问，再也不需要折腾网络、账户和各种技术术语。
- 修复了重新启动时夜间主题失效的问题
- 修复回答生成时无法切换会话的问题
- 修复消息编辑时的卡顿问题
