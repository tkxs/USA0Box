...
1. 修復 Latex 渲染問題
2. 修復左邊欄頂部不能拖動的問題
3. 修復 ZeroBoxAI 錯誤信息展示
4. 增加初始化過程日志展示

### v1.12.0 - 2025.04.24

1. ZeroBox AI 支援 Gemini 多模態輸出
...
1. 新增聯網問答功能。現在 ZeroBox AI Models 和 Gemini 2.0 flash(API) 可以參考網際網路的即時資訊進行回答，並在訊息中列出參考資訊來源
2. 新增查看訊息生成時的首字延遲時間（first-token latency），可在設定中開啟
3. 新增傳送圖片的預處理能力。現在你可以傳送更多格式的圖片（例如 svg、gif），ZeroBox 會自動轉化成模型可以接收的圖片格式，並且自動調節圖片大小以符合模型要求
4. 雙擊主題名稱可以快速彈出主題列表
...
1. 現在您可以傳送網頁連結了。ZeroBox 會自動擷取網頁內容並將其納入聊天脈絡中。此功能適用於所有模型。
2. 現在您可以向任何模型傳送文字檔案。ZeroBox 會在本地解析檔案內容並將其納入聊天脈絡中。
...
1. 現在 ZeroBox AI 服務的用戶可以選擇更多的模型
2. 新增阿拉伯語（العربية）
3. 優化了 LaTeX 的樣式
4. 修復了小機率情況下遺失資料的問題
5. 修復了其他一些小問題
...
- ZeroBox 網頁版本現在也能訪問本地 Ollama 服務。
...
3. 現在您從 ZeroBox 複製的文字，將以純文字形式複製到剪貼簿（不再包含背景顏色，這個長久以來的小錯誤終於得到修復）
...
- 新增圖片生成功能（Image Creator），現在您可以在 ZeroBox 中生成圖片了。由 Dall-E-3 模型提供支持。
- 優化一些互動問題
...
- 內建AI服務 “ZeroBox AI”，開箱即用，直接高速訪問，再也不需要折騰網路、帳號和各種專業術語。
- 修正了重新啟動時夜間主題失效的問題
- 修正回答產生時無法切換會話的問題
- 修正訊息編輯時的卡頓問題
