...
async function initAuthenticatedAfetch(): Promise<ReturnType<typeof createAuthenticatedAfetch>> {
  ...
  authenticatedAfetchPromise = (async () => {
    _authenticatedAfetch = createAuthenticatedAfetch({
      ...
      getTokens: async () => {
        // SUB2API owns the application's account tokens. Never forward them to
        // legacy Chatbox account endpoints.
        return null
      },
      refreshTokens: async () => {
        throw new Error('Legacy Chatbox account authentication is disabled')
      },
      clearTokens: async () => {
        // This client only serves legacy Chatbox account endpoints. The persisted
        // auth store now belongs to SUB2API, so a Chatbox 401 must not sign the
        // user out of SUB2API.
        log.warn('Legacy Chatbox authentication failed; preserving SUB2API session')
      },
    })
