...
import { createCustomProviderModel } from '@shared/providers/utils'
...
export default class RegistrySettingUtil extends BaseConfig implements ModelSettingUtil {
  ...
  protected async listProviderModels(settings: ProviderSettings): Promise<ProviderModelInfo[]> {
    ...
    const modelInstance = mergedSettings.sub2apiKeyId
      ? createCustomProviderModel(createConfig, definition.type, dependencies)
      : definition.createModel(createConfig)
