...
describe('getModel', () => {
  ...
  it('routes built-in providers through the SUB2API gateway when a group key is selected', () => {
    const sessionSettings: SessionSettings = {
    ...
    expect((model as CustomOpenAI).options).toMatchObject({
      apiKey: 'sk-group-key',
      apiHost: 'http://localhost:18080',
      apiPath: '/chat/completions',
    })
